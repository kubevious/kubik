...
class TargetProcessor
{
    ...
    _filterLogicItem(targetSelector, prev, item)
    {
        return Promise.resolve()
            ...
            .then(shouldAdd => {
                ...
                return this._filterLogicItemByCustomFilter(targetSelector, prev, item);
            })
            ;
    }
    ...
    {
        return this._doesAnyMatch(targetSelector, prev, item, targetSelector._nameFilters, 
            (value) => {
    ...
    {
        return this._doesAllMatch(targetSelector, prev, item, targetSelector._customFilters, 
            (value) => {
                return value;
            });
    }
    ...
    _doesAnyMatch(targetSelector, prev, item, matchers, cb)
    {
        if (matchers.length == 0) {
            return true;
        }
        return this._tryNextAnyMatcher(targetSelector, prev, item, matchers, cb, 0);
    }

    _tryNextAnyMatcher(targetSelector, prev, item, matchers, cb, index)
    {
        ...
        return this._resolveMatcherValue(matcher, prev, item)
            .then(matcherValue => {
            ...
            .then(didMatch => {
                ...
                return this._tryNextAnyMatcher(targetSelector, prev, item, matchers, cb, index + 1);
            })
    }

    _doesAllMatch(targetSelector, prev, item, matchers, cb)
    {
        if (matchers.length == 0) {
            return true;
        }

        return this._tryNextAllMatcher(targetSelector, prev, item, matchers, cb, 0);
    }

    _tryNextAllMatcher(targetSelector, prev, item, matchers, cb, index)
    {
        if (index >= matchers.length) {
            return true;
        }
        ...
        return this._resolveMatcherValue(matcher, prev, item)
            ...
            .then(didMatch => {
                ...
                return this._tryNextAllMatcher(targetSelector, prev, item, matchers, cb, index + 1);
            })
    }

    _resolveMatcherValue(matcher, prev, item)
    {
        if (_.isFunction(matcher))
        {
            var params = {
                prev: prev,
                item: new TargetItem(item, this._state)
            }
            return Promise.resolve(matcher(params));
        }
        else
        {
